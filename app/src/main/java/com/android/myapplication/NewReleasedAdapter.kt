...
class NewReleasedAdapter(
    private var items: List<BookItem>, // 책 목록 데이터 리스트
    ...
    class ViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        val bookCoverImageView: ImageView = itemView.findViewById(R.id.mf_book_cover) // 책 표지 이미지뷰
    ...
    // 새로운 ViewHolder를 생성하는 메서드 (RecyclerView가 새로운 아이템을 만들 때 호출됨)
    ...
    fun updateBooks(newItems: List<BookItem>) {
        items = newItems
        notifyDataSetChanged() // RecyclerView를 갱신하여 최신 데이터 반영
    }
}
